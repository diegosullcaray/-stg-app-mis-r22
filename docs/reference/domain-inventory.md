...
| `/app/consulta-fen` | consulta de riesgo territorial CENEPRED | módulo lazy top-level; usa `table.regular` mediante `ModRepService` |
...
`/app/consulta-fen` usa `ModRepService.getRegularTableResult` con código `CON_AGRO_FEN`. La búsqueda por UBIGEO envía `{ col: 0, val }` y la búsqueda por distrito `{ col: 3, val }`; `val` permanece como texto y UBIGEO conserva ceros iniciales. El alias es `body.resultado`, con `headers` vacío y `data` como arreglo de filas con `cod_ubi`, `des_dep`, `des_prov`, `des_dist`, `exp_mas`, `exp_inu`, `exp_seq` y `exp_pre`.

Las cabeceras y estilos de `stg-table2` son locales porque backend no entrega configuración visual. La pantalla usa un selector Material único: Ubigeo=0, Departamento=1, Provincia=2 y Distrito=3 (predeterminado). Solo `col: 0` y `col: 3` tienen contrato confirmado en este repositorio. Departamento y Provincia se muestran, pero el campo y Buscar se deshabilitan con un aviso de indisponibilidad; la lógica también impide enviar esos valores hasta confirmar su soporte. No se deduce una capacidad backend del orden de las columnas.

Los filtros ocupan todo el ancho. Se retiraron tabs y cards superiores; la selección aparece en una franja compacta de la matriz y mantiene la recomendación comercial para riesgo predominante `Alto` o `Muy Alto`. Una sola fila se selecciona automáticamente; con varias se espera selección explícita. Cambiar de filtro limpia búsqueda, selección y resultados. La fecha `12 set. 2026` y la recomendación siguen siendo reglas locales pendientes de metadata backend.

La matriz usa `stg-table2` con páginas locales de 10 filas mediante `stg-paginator`, total de todas las filas recibidas y reinicio por búsqueda. Paginar no consulta backend ni muta filas. El paginador aparece solo con más de 10 registros; carga, vacío y error limpian la página visible. Se conserva la selección al paginar. El pie es hermano del contenedor desplazable, con `flex: 0 0 auto`; la tabla usa el alto restante y scroll propio. Se eliminó la altura móvil fija de 548 px, que podía desplazar el pie fuera del viewport junto a los antiguos bloques superiores. El alto del consumidor se limita al viewport descontando los 48 px del shell; en pantallas demasiado bajas se permite scroll del contenido completo.
